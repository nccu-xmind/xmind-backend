...
class FGScUAppNameProbe
{
    ...
    public function perform()
    {
        ...
        $sql = 'INSERT INTO `probe_fgapp_log`(`userID`, `UHID`, `Type`, `TriggeredTimes`, `PackageName`, `Activity`) '
                . 'VALUES (:userID, :UHID, 2, :TriggeredTimes, :PackageName, :Activity)';
        try {
            ...
            if ($stmt) {
                ...
                $err_msg = json_encode('[' . date("Y-m-d H:i:s") . '] FGScUAppNameProbe Job Error:' .
...
//            echo $exc->getMessage();
            $err_msg = '[' . date("Y-m-d H:i:s") . '] FGScUAppNameProbe Job Exception:' .
