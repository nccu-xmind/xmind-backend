...
class CallLogProbe
{
    ...
    public function perform()
    {
        ...
        $sql = 'INSERT INTO `probe_call_log`(`userID`, `UHID`, `TriggeredTimes`, `CallDate`, `CallDuration`) '
                . 'VALUES (:userID, :UHID, :TriggeredTimes, :CallDate, :CallDuration)';
        try {
            ...
            if ($stmt) {
                ...
                $stmt->bindParam(':CallDate', $call_date, \PDO::PARAM_STR);
                $stmt->bindParam(':CallDuration', $call_duration, \PDO::PARAM_STR);
                ...
                foreach ($this->args as $probe_log) {
                    ...
                    $call_date = date("Y-m-d H:i:s", intval($probe_log["cdt"]));
                    $call_duration = $probe_log["cdu"];
                    $stmt->execute();
                }
            } else {
                $err_msg = json_encode('[' . date("Y-m-d H:i:s") . '] CallLogProbe Job Error:' .
            ...
            $err_msg = '[' . date("Y-m-d H:i:s") . '] CallLogProbe Job Exception:' .
